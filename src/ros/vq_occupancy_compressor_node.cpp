...
public:
    VqOccupancyCompressorNode()
        : rclcpp::Node("vq_occupancy_compressor_node") {
        declareParameters();
        loadParameters();
        ...
        RCLCPP_INFO(get_logger(),
                    "VqOccupancy Compressor Node initialized - Input: %s, Voxel: %.3f, Block: %d",
                    input_file_.c_str(), voxel_size_, block_size_);
        setupPublishers();
        compressAndPublish();
    }

    ~VqOccupancyCompressorNode() override = default;
...
private:
    ...
    void setupPublishers() {
        pattern_dict_pub_ = this->create_publisher<vq_occupancy_compressor::msg::PatternDictionary>(
    ...
    vq_occupancy_compressor::config::CompressorConfig configFromParameters() const {
        vq_occupancy_compressor::config::CompressorConfig config;
    ...
    void compressAndPublish() {
        auto setup = vq_occupancy_compressor::config::buildCompressionSetup(configFromParameters());
        auto errors = vq_occupancy_compressor::config::validateCompressionSetup(setup);
        if (!errors.empty()) {
        ...
        const bool success = vq_occupancy_compressor::services::runCompression(
            setup,
            [this](const PCCCompressionReport& report,
                   vq_occupancy_compressor::io::CompressionReportBuilder&) {
    ...
    vq_occupancy_compressor::msg::PatternDictionary createPatternDictionaryMessage(
        const PCCCompressionReport& report) {
        vq_occupancy_compressor::msg::PatternDictionary msg;
    ...
    rclcpp::Publisher<vq_occupancy_compressor::msg::PatternDictionary>::SharedPtr pattern_dict_pub_;
...
int main(int argc, char** argv) {
    ...
    try {
        auto node = std::make_shared<VqOccupancyCompressorNode>();
        rclcpp::spin(node);
    } catch (const std::exception& e) {
        RCLCPP_ERROR(rclcpp::get_logger("vq_occupancy_compressor_node"),
